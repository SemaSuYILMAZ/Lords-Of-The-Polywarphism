...
def draw_warrior(screen, position, square_size, color, warrior_name):
    ...
    text_rect = text.get_rect(center=(position[0] * square_size + square_size // 2, position[1] * square_size + square_size // 2))
...
def ai_place_warriors(player_id, players, rows, cols, valid_positions, warriors_matrix):
    ...
    if available_positions:
        ...
        players[player_id]['warrior_name'] = "T"
        players[player_id]['warrior_position'] = (x, y)
        warriors_matrix[x][y] = {'player_id': player_id, 'player_name': players[player_id]['name'], 'warrior_name': "T", 'color': players[player_id]['color']}
        return True
    else:
        return False
